...
-- 5. 翻译插件配置加载：xingshou/lua/plugins/translator.lua
-- 核心定位：Neovim 的「中英文翻译工具」，支持划词/行/手动输入翻译，阅读英文文档/代码时快速查词
-- 核心功能与特性（基于 vim-translator 插件，新手友好、开箱即用）：
-- - 多模式翻译：划词翻译（可视模式选中文字）、行翻译（当前行）、输入翻译（手动输入内容）
-- - 无需额外配置：默认使用百度翻译引擎，无需申请 API 密钥，首次使用自动生效
-- - 弹窗显示结果：翻译结果在弹窗中展示，不遮挡编辑区，看完自动关闭（不打断编码流程）
-- - 自动语言识别：支持英文→中文、中文→英文双向翻译，无需手动切换源语言
-- 关键功能详解：
-- - 翻译模式与快捷键（<leader> 默认为空格，易记易用）：
--   - 划词翻译：可视模式（按 v）选中文字 → 按 <leader>t，即时显示翻译结果
--   - 行翻译：普通模式下按 <leader>tl，翻译当前整行内容（适合阅读英文文档）
--   - 输入翻译：普通模式下按 <leader>ti，底部输入框输入单词/句子 → 回车显示翻译
-- - 基础配置（已在 translator.lua 中预设，新手无需修改）：
--   - 翻译引擎：默认百度翻译（稳定、免费，支持多语言）
--   - 目标语言：默认简体中文（可在 translator.lua 中改为 en 切换英文翻译）
--   - 弹窗样式：宽度 80、高度 10，适配大多数屏幕，文字清晰不拥挤
-- 使用场景：
-- - 阅读英文代码注释：选中陌生单词，按 <leader>t 快速查含义，不用切换浏览器
-- - 查看英文插件文档：打开插件 README.md，按 <leader>tl 翻译整行，快速理解功能
-- - 学习英文语法：遇到长句时，按 <leader>ti 输入句子，查看拆分翻译和语法结构
-- 注意点：
...
-- | 翻译功能                | 划词选中→<leader>t / <leader>tl行翻译 / <leader>ti输入翻译 | xingshou/lua/plugins/translator.lua | 无需API密钥，弹窗显示翻译结果          |
...
-- 7. 翻译功能失效（如按 <leader>t 无反应）：
--    - 执行 :Lazy 查看 vim-translator 插件是否已安装，未安装则点击 Install
--    - 检查快捷键是否冲突：在 translator.lua 中修改 keys 字段的快捷键（如改为 <leader>tr）
--    - 网络问题：翻译需联网，确认网络通畅后重新触发翻译